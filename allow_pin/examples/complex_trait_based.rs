...
use allow_pin::{ErrorCode, command, trait_based::*};
...
fn api<
    ...
    const RW_BUFFER: u32,
>(
    mut ro_buffer: Pin<&mut RO>,
    mut rw_buffer: Pin<&mut RW>,
) -> Result<(), ErrorCode> {
    ro_buffer.as_mut().allow_ro(DRIVER_NUM, RO_BUFFER)?;
    rw_buffer.as_mut().allow_rw(DRIVER_NUM, RW_BUFFER)?;
    // Subscribe goes here.
    ...
    // Yield goes here.
    // Perform unallows.
    AllowRo::unallow(ro_buffer);
    AllowRw::unallow(rw_buffer);
    Ok(())
}
...
>(
    ...
    let ro_buffer: Pin<&mut Buffer<[u8; RO_LEN]>> = pin!(Buffer::<[u8; RO_LEN]>::from(ro_data));
    let mut rw_buffer: Pin<&mut Buffer<[u8; RW_LEN]>> = pin!(Buffer::from([0u8; RW_LEN]));
    api::<_, _, DRIVER_NUM, RO_BUFFER, RW_BUFFER>(ro_buffer, rw_buffer.as_mut())?;
    match AllowRw::unallow(rw_buffer) {
        Some(buf) => Ok(*buf),
        None => Err(1),
    }
}

static BUFFER: [u8; 40] = [0; 40];
